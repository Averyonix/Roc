...
layout(push_constant, scalar) uniform PushConstants { SceneRenderBinPassInput pc; };

layout(local_size_x = SCENE_RENDER_BIN_PASS_LOCAL_SIZE,
       local_size_y = SCENE_RENDER_BIN_PASS_LOCAL_SIZE, local_size_z = 1) in;
void main()
{
    ...
    aabb2f32 tile;
    tile.min = vec2f32(gl_GlobalInvocationID.xy) * vec2f32(SCENE_RENDER_FINE_BIN_SIZE);
    tile.max = tile.min + vec2f32(SCENE_RENDER_FINE_BIN_SIZE);
    ...
    while (true) {
        ...
        in_slot += 1;
        if (in_slot == SCENE_RENDER_QUADS_PER_BIN) {
    ...
    if (out_slot < coarse_bin_info._.depth) {
        fine_bin[out_slot]._ = SCENE_RENDER_QUAD_INDEX_TYPE(0);
    }
}
